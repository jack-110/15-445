...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw Exception("expect the number of args to be 1");
  }

  std::shared_ptr<StringExpression> result;
  if (func_name == "upper") {
    result = std::make_shared<StringExpression>(std::move(args[0]), StringExpressionType::Upper);
  } else if (func_name == "lower") {
    result = std::make_shared<StringExpression>(std::move(args[0]), StringExpressionType::Lower);
  } else {
